...
export default function Booking() {
  ...
  useEffect(() => {
    if (stateCurrOrder?.CheckInDate && stateCurrOrder?.CheckOutDate) {
      const [day, month, year] =
        stateCurrOrder.CheckInDate.split("/").map(Number);
      const checkInDate = new Date(year, month - 1, day);
      const [day2, month2, year2] =
        stateCurrOrder.CheckOutDate.split("/").map(Number);
  ...
  const getSlideImage = async () => {
    try {
      const new_slide_images = await Promise.all(
        property.currentLocation_images.map(async (id, index) => {
  ...
  useEffect(() => {
    const totalSum = stateCurrOrder?.amenities.reduce(
      (accumulator, amenity) => {
  ...
  const getAmenityTitle = async () => {
    if (
      stateCurrOrder &&
      stateCurrOrder.amenities &&
      Array.isArray(stateCurrOrder.amenities)
    ) {
      const amenitytitle = await Promise.all(
        stateCurrOrder.amenities.map(async (item, index) => {
  ...
  const WhatsappMessage = () => {
    ...
    let msg =
      "Hello+I%27m+" +
      user.name +
      "%2C+I+would+like+to+enquire+about+following+property%0D%0AProperty+%3A+" +
      stateCurrOrder.Title +
      "%0D%0ALocation+%3A+" +
      stateCurrOrder.Location +
      "%0D%0ACheckInDate+%3A+" +
      stateCurrOrder.CheckInDate +
      "%0D%0ACheckOutDate+%3A+" +
      stateCurrOrder.CheckOutDate +
      "%0D%0ARoom+Type+%3A+" +
      stateCurrOrder.RoomType +
      "%0D%0ANumber+of+Adult+%3A+" +
      stateCurrOrder.adultNumber +
      "%0D%0ANumber+of+Child+%3A+" +
      stateCurrOrder.childNumber +
      "%0D%0AAmenities+%3A+" +
      amenityTitle;

    window.open(
      "https://api.whatsapp.com/send?phone=919136886650&text=" + msg,
      "_blank",
      "noreferrer"
    );

    const requiredFormatInDate = convertDateFormat(stateCurrOrder.CheckInDate);
    const requiredFormatOutDate = convertDateFormat(
      stateCurrOrder.CheckOutDate
    );
  ...
  const sendEmail = () => {
    ...
    emailjs
      .send("service_xocsixq", "template_5jokq89", param, "hxrJoDY6Iai13B0RZ")
      .then(
        (result) => {
        ...
        <div className="flex gap-x-2 flex-col md:flex-row">
          <div className="md:w-2/3 p-4 rounded border-2 border-[#6ACDE9]">
            ...
            <div className="mt-4">
              ...
              <h1 className="text-lg">
                Total guest :{" "}
                {stateCurrOrder?.adultNumber + stateCurrOrder?.childNumber}
              </h1>
              <h1 className="text-lg">
                Adult : {stateCurrOrder?.adultNumber} Children :{" "}
                {stateCurrOrder?.childNumber}
              </h1>
              <div className="flex">
                <div className="w-1/2 pr-2 py-2 text-white font-medium">
                  <div className="bg-[#6ACDE9] md:text-xl p-4 rounded-lg ">
                    <h1 className="">Check In Date</h1>
                    {stateCurrOrder?.CheckInDate}
                  </div>
                </div>
                <div className="w-1/2 pl-2 py-2 text-white font-medium">
                  <div className="bg-[#6ACDE9] md:text-xl p-4 rounded-lg ">
                    <h1 className="">Check Out Date</h1>
                    {stateCurrOrder?.CheckOutDate}
                  </div>
                </div>
              </div>
          ...
          <div className="md:w-1/3 rounded border-2 border-[#6ACDE9]">
            <div className="p-4 border-b-2">
              <h1 className="text-xl text-[#949494] font-medium">
                Price Details
              </h1>
            </div>
            <div className="p-4 border-b-2 flex justify-between items-center">
              <div>
                <h1 className="text-lg font-medium">Stay Amount</h1>
                <h1>
                  {stateCurrOrder?.adultNumber} Adults x {numberOfNights} Nights
                </h1>
              </div>
              <div>
                <h1 className="text-[#268F43] font-bold">RS {price * numberOfNights}/-</h1>
              </div>
            </div>
            <div className="p-4 border-b-2 flex justify-between items-center">
              <div>
                <h1 className="text-lg font-medium">Total Service Charges</h1>
                <h1>( {stateCurrOrder?.amenities.length} Items)</h1>
              </div>
              <div>
                <h1 className="text-[#268F43] font-bold">
                  RS {amenitiesPrice} /-
                </h1>
              </div>
            </div>
            <div className="p-4 border-b-2 flex justify-between items-center">
              <div>
                <h1 className="text-lg font-medium">Amount to be Paid</h1>
              </div>
              <div>
                <h1 className="text-[#268F43] font-bold">
                  RS {(price * numberOfNights) + amenitiesPrice}/-
                </h1>
              </div>
            </div>
            <div className="p-4 border-b-2">
              <button
                ...
                className=" bg-[#F79489] w-full md:text-xl text-xl py-2 rounded font-medium text-white"
              >
                Book Now
              </button>
            </div>
            <div className="p-4">
              <h1 className="font-medium">
                Our Team will shortly contact you with the availabilty of our
                Homestays
              </h1>
            </div>
          </div>
      ...
      >
        ...
        <Modal.Body>
          <div className=" w-full">
            <h1 className="font-medium text-center text-lg underline decoration-[#F79489] underline-offset-8 decoration-4">
              Send Booking Request
            </h1>
            <div className="flex gap-x-2 mt-4 flex-row   ">
              <div
                className="w-1/2 p-4 rounded border-2 border-[#6ACDE9] cursor-pointer"
